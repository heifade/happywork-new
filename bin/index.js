...
var archiveDir = require('archive-dir');
...
function unzipTemplate(template, path$$1) {
    var dir = path.resolve(__dirname, "./template/");
    var zipFile = dir + "/" + template + ".zip";
    if (!fs.existsSync(zipFile)) {
        var list = fs.readdirSync(dir)
            .map(function (item) { return path.basename(item, ".zip"); })
            .join("\n");
        console.log(chalk.red("\u6A21\u677F" + template + "\u4E0D\u5B58\u5728\uFF0C\u6A21\u677F\u5217\u8868\uFF1A\n" + list));
    }
    archiveDir.unzipDir(zipFile, path$$1);
}

function addNewWebCommand() {
    commander
        .command("web")
        .option("--path <n>", "项目路径", process.cwd())
        .option("--template <n>", "模板名称", "webEmpty")
        .description("新建Web项目")
        .action(function (pars) {
        unzipTemplate(pars.template, pars.path);
    });
}

function addNewNodeCommand() {
    commander
        .command("node")
        .option("--path <n>", "项目路径", process.cwd())
